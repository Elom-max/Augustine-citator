#!/usr/bin/env node
// PDF -> reflowable EPUB, command line.
//   node pdf2epub/cli.mjs livre.pdf [sortie.epub] [--title "..."] [--author "..."] [--lang fr]
//   node pdf2epub/cli.mjs *.pdf            (conversion en lot, un .epub par PDF)
...
async function convertOne(pdfjs, inputPath, opts) {
  ...
  const book = buildBook(pages, {
    title: opts.title || info.Title || basename(inputPath, extname(inputPath)),
  ...
  const outPath =
    opts.out ||
    join(dirname(inputPath), basename(inputPath, extname(inputPath)) + '.epub');
  await writeFile(outPath, epub);
  const kb = (epub.length / 1024).toFixed(0);
  console.log(
    `  ✓ ${outPath}  (${book.stats.pages} pages, ${book.stats.chapters} chapitres, ${book.stats.chars} caractères, ${kb} Ko)`
  );
  return book;
}
...
async function main() {
  ...
  if (!inputs.length) {
    console.error(
      'Usage : node pdf2epub/cli.mjs <fichier.pdf> [sortie.epub] [--title "..."] [--author "..."] [--lang fr]'
    );
    process.exit(1);
  }
